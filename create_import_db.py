...
def import_csv_db():
    ...
    campus_dicts = read_csv('app/database/inputs/campus_instituto.csv', parse_dates=["ano_fundacao"], na_filter=False, sep=';', encoding='UTF-8').to_dict(orient='records')
    curso_dicts = read_csv('app/database/inputs/curso.csv', parse_dates=["data_fundacao"], na_filter=False, sep=';', encoding='UTF-8').to_dict(orient='records')
    discente_dicts = read_csv('app/database/inputs/discentes.csv', na_filter=False, sep=';', encoding='UTF-8').to_dict(orient='records')
    docente_dicts = read_csv('app/database/inputs/docentes.csv', parse_dates=["data_nascimento"], na_filter=False, sep=';', encoding='UTF-8').to_dict(orient='records')
    ...
    tecnico_dicts = read_csv('app/database/inputs/tecnico.csv', parse_dates=["data_nascimento"], na_filter=False, sep=';', encoding='UTF-8').to_dict(orient='records')
